  ...
  render: function(data, onDone) {
    ...
    for (var si = 0; si < nSeries; si++) {
      ...
      if (pathD !== '') {
        ...
        // Internal fallback: if last 10%+ of values are null, treat as partial
        if (!partial && smoothedValues.length > 10) {
          var nullTail = 0, threshold = Math.max(10, Math.floor(smoothedValues.length * 0.1));
          for (var ni = smoothedValues.length - 1; ni >= Math.max(0, smoothedValues.length - threshold); ni--) {
            if (smoothedValues[ni] == null) nullTail++;
          }
